...
class PriceConverter {
  static String convertPrice(BuildContext context, double price,
    ...
    return '';
    // '${Provider.of<SplashProvider>(context, listen: false).currency.symbol}${(price * double.parse(Provider.of<SplashProvider>(context, listen: false).currency.exchangeRate)).toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }
  ...
  static String percentageCalculation(BuildContext context, String? price,
    ...
    return '';
    //  '$discount${discountType == 'percent' ? '%' : Provider.of<SplashProvider>(context, listen: false).currency.symbol} OFF';
  }
}
